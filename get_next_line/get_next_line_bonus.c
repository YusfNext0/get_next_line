...
/*   Updated: 2024/02/17 16:33:00 by ynachat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	allocate(char **str, int fd, size_t *n)
{
...
char	*fline(char *str, char **buffer)
{
	char	*fstr_stock;
	int		i;
	int		j;
...
char	*get_next_line(int fd)
{
	...
	if (!line[fd] || !line[fd][0])
		return (free(buffer), free(line[fd]), line[fd] = NULL, NULL);
	return_str = fline(line[fd], &buffer);
